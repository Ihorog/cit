...
- 💬 **Чат-інтерфейс** — повна інтеграція з CIT сервером для спілкування з OpenAI
...
- 🔌 **Real-time статус** — моніторинг підключення до CIT сервера
...
### Інтеграція з CIT сервером

Для роботи чату в розділі "Казкар" потрібен запущений CIT сервер.

**Крок 1: Запустіть CIT сервер**

```bash
# В новому терміналі, перейдіть до кореневої директорії
cd ..

# Встановіть OPENAI_API_KEY (якщо ще не встановлено)
export OPENAI_API_KEY=sk-...

# Запустіть CIT сервер
python server/cit_server.py
```

CIT сервер запуститься на порту 8790 та буде доступний за адресою `http://127.0.0.1:8790`.

**Крок 2: Налаштуйте API URL (опціонально)**

За замовчуванням веб-інтерфейс підключається до `http://127.0.0.1:8790`. Щоб змінити URL:

```bash
# Створіть файл .env.local у директорії web/
cp .env.local.example .env.local

# Відредагуйте .env.local та вкажіть ваш CIT API URL
# Наприклад, для Tailscale VPN:
# NEXT_PUBLIC_CIT_API_URL=http://100.x.x.x:8790
```

**Крок 3: Перезапустіть dev сервер**

```bash
npm run dev
```

Тепер секція "Казкар" має повнофункціональний чат з інтеграцією OpenAI!
