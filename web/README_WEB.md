...
**ВАЖЛИВО:** При деплої на Vercel **ОБОВ'ЯЗКОВО** вкажіть у налаштуваннях проєкту:

- **Framework Preset**: Next.js
- **Root Directory**: `web` ⚠️ **БЕЗ ЦЬОГО ДЕПЛОЙ НЕ ПРАЦЮВАТИМЕ**
- **Build Command**: `npm run build` (автоматично визначається)
- **Output Directory**: `.next` (автоматично визначається)
- **Install Command**: `npm install` (автоматично визначається)

> **Примітка:** Файл `vercel.json` у корені репозиторію є мінімальним. Всі налаштування білду мають бути виставлені через Vercel Dashboard у розділі "Project Settings → General → Build & Development Settings".
...
3. **⚠️ КРИТИЧНО:** У полі "Root Directory" виберіть `web`
4. Framework буде автоматично визначений як Next.js
5. Натисніть **Deploy**
...
3. Якщо там вказано `cit` або будь-що інше крім `web` - **це помилка**
4. Змініть на `web` (саме слово "web" без лапок)
5. Натисніть **Save**
6. Перейдіть у розділ **Deployments**
7. Знайдіть останній деплой і натисніть **Redeploy**

**Проблема:** Білий екран або помилка "404: NOT_FOUND"

**Рішення:**
1. Перевірте що в Vercel Project Settings → General встановлено:
   - **Root Directory**: `web` (не порожнє!)
   - **Framework Preset**: Next.js
2. Якщо параметр "Root Directory" був пустим або містив неправильне значення:
   - Встановіть `web`
   - Перейдіть у Deployments
   - Натисніть "Redeploy" на останньому деплої
3. Переконайтесь що `vercel.json` у корені репо не містить `buildCommand`, `installCommand` або `outputDirectory` (ці параметри не підтримуються)

**Проблема:** Build fails або "Command not found"

**Рішення:**
1. Переконайтесь що Root Directory = `web`
2. Build Command має бути `npm run build` (без `cd web`)
3. Install Command має бути `npm install` (без `cd web`)
