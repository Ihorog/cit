...
При деплої на Vercel налаштування здійснюються автоматично через `vercel.json`:

- **Framework Preset**: Next.js (визначається автоматично)
- **Root Directory**: Залиште порожнім або не вказуйте (команди збірки автоматично переходять в `web/`)
- **Build Command**: `cd web && npm run build` (налаштовано в vercel.json)
- **Install Command**: `cd web && npm install` (налаштовано в vercel.json)

> **Примітка:** Конфігурація збірки знаходиться в `vercel.json` у корені репозиторію. Команди автоматично виконуються в директорії `web/`, тому вручну встановлювати Root Directory не потрібно.
...
3. Framework буде автоматично визначений як Next.js
4. **Важливо:** Поле "Root Directory" залиште **порожнім** (не вказуйте `web` або `cit`)
5. Натисніть **Deploy** - збірка відбудеться автоматично згідно з `vercel.json`
...
**Проблема:** "Вказаний кореневий каталог 'cit' не існує" або "The specified root directory 'cit' does not exist"

**Рішення:**
1. Перейдіть у Vercel Dashboard → Project Settings → General
2. У розділі "Build & Development Settings" знайдіть поле **Root Directory**
3. Якщо там вказано `cit` або будь-що інше - **очистіть це поле** (залиште порожнім)
4. Натисніть **Save**
5. Перейдіть у розділ **Deployments**
6. Знайдіть останній деплой і натисніть **Redeploy**

> **Пояснення:** Файл `vercel.json` містить команди які автоматично переходять в директорію `web/`. Root Directory повинен бути порожнім.

**Проблема:** Білий екран або помилка "404: NOT_FOUND"

**Рішення:**
1. Перевірте що в Vercel Project Settings → General встановлено:
   - **Root Directory**: порожнє поле (не `web`, не `cit`)
   - **Framework Preset**: Next.js
2. Переконайтесь що `vercel.json` у корені репо містить `buildCommand` та `installCommand` з `cd web`
3. Перейдіть у Deployments → Redeploy

**Проблема:** Build fails або "Command not found"

**Рішення:**
1. Переконайтесь що Root Directory порожній (не заповнений)
2. Перевірте що `vercel.json` містить правильні команди:
   ```json
   {
