/**
 * REPORT-TASK - reports individual task execution result
 */

import fs from 'fs/promises';

function parseArg(name) {
  const arg = process.argv.find(a => a.startsWith(`--${name}`));
  if (!arg) return undefined;
  if (arg.includes('=')) return arg.split('=').slice(1).join('=');
  const idx = process.argv.indexOf(arg);
  return process.argv[idx + 1];
}

const conductorId = parseArg('conductor-id');
const nodeId = parseArg('node-id');
const taskId = parseArg('task-id');
const resultPath = parseArg('result');
const status = parseArg('status') || 'unknown';

if (!conductorId || !nodeId || !taskId) {
  console.error('Usage: node report-task.js --conductor-id <id> --node-id <id> --task-id <id> --result <path> --status <status>');
  process.exit(1);
}

let result = null;
if (resultPath) {
  try {
    result = JSON.parse(await fs.readFile(resultPath, 'utf-8'));
  } catch (err) {
    console.error(`Failed to read result file ${resultPath}: ${err.message}`);
    result = { error: `Could not read result file: ${resultPath}` };
  }
}

const report = {
  conductor_id: conductorId,
  node_id: nodeId,
  task_id: taskId,
  status,
  result,
  reported_at: new Date().toISOString()
};

console.log(JSON.stringify(report, null, 2));
